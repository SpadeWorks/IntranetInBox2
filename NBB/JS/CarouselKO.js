...
var CarouselLandingKOModel = (function () {
    ...
    CarouselLandingKOModel.CarouselLandingViewModel = function () {
        ...
        self.GetCarouselData = function (limit) {
            ...
            NBB.ServerDateTime().then(function (date) {
                var siteUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('Organizational Announcements')" +
                    "/items?$select=Id,Title,AnnouncementDetail,EmployeeDepartment,Expires,ContentPriority&$filter=OData__ModerationStatus eq 0 and Expires ge '" + date + "'&$top=" + limit + "&$orderby=ContentPriority asc";
                        ...
                        if (data.value.length > 0) {
                            $.each(data.value, function (position, item) {
                                ...
                                self.GetBannerImage(item.Id).then(function (bannerImage) {
                                    ...
                                    if (!isExpired(date)) {
                                        if (itemcount === 0) {
                                            self.CarouselArray.push(new CarouselArrayItemPush(item.Id, item.Title, bannerImage, (item.EmployeeDepartment[0] != undefined ? item.EmployeeDepartment[0].Label:""), itemcount, "active", date, readMore));
                                            self.IndicatorArray.push(new IndicatorArrayItemPush(itemcount, "active"));
                                            itemcount++;
                                        }
                                        else {
                                            self.CarouselArray.push(new CarouselArrayItemPush(item.Id, item.Title, bannerImage, (item.EmployeeDepartment[0] != undefined ? item.EmployeeDepartment[0].Label:""), itemcount, "", date, readMore));
        ...
        self.GetCarouselLimit = function () {
            var deferred = $.Deferred();
            var siteUrl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('Site Configuration')/items?$select=Title,Value&$filter=Title eq 'Organization Announcement Carousel Limit' or Title eq 'Carousel Default Image'";
            $.ajax({
                url: siteUrl,
                method: 'GET',
                headers: { 'Accept': 'application/json; odata=nometadata' },
